...
function Journal() {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            let response;
            if (isEditing) {
                response = await axios.put(`${apiUrl}/journalRoute/upDate/${editingId}`, formData, {
                    headers: {
                ...
                response = await axios.post(`${apiUrl}/journalRoute/create`, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                });
            ...
            if (response.data) {
                ...
                setIsEditing(false);
                setEditingId(null);
                toast.success(isEditing ? 'Journal upDated successfully!' : 'Journal created successfully!');
            }
        } catch (error) {
            console.error('Error submitting form:', error);
            toast.error(`Error ${isEditing ? 'updating' : 'creating'} journal. Please try again.`);
        }
    };

    const getJournalById = async (id) => {
        try {
            const response = await axios.get(`${apiUrl}/journalRoute/get/${id}`);
            ...
            if (data) {
                ...
                if (data) {
                    ...
                    setValue('journalNo', data?.journalNo);
    ...
    const handleEdit = (journal) => {
        ...
        getJournalById(journal._id);
    };

    const handleDelete = async (id) => {
        try {
            const response = await axios.delete(`${apiUrl}/journalRoute/delete/${id}`);
            if (response.data) {
                getJournalList(); // Refresh the list after deletion
                toast.success('Journal deleted successfully!');
            }
        } catch (error) {
            console.error('Error deleting journal:', error);
            toast.error('Error deleting journal. Please try again.');
        }
    };
    ...
    const getJournalList = async () => {
        try {
            const response = await axios.get(`${apiUrl}/journalRoute/getAll`);
            console.log(response?.data?.data, "response journal list");
    ...
    const filteredPayments = payments.filter(payment =>
        (payment?.journalNo?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
        (payment?.fromAccount?._id?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
        (payment?.toAccount?._id?.toLowerCase() || '').includes(searchTerm.toLowerCase())
    );
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-col sm:flex-row w-full items-start sm:items-center gap-3 mb-4">
                                    ...
                                    <div className="w-full sm:w-3/4">
                                        <input
                                            type="text"
                                            {...register("journalNo", { required: "Payment number is required" })}
                                            placeholder="Enter payment number"
                                            className="w-full border border-[#C0D5FF] rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                        />
                                        {errors.journalNo && (
                                            <p className="text-red-500 text-sm mt-1">{errors.journalNo.message}</p>
                                        )}
                                    </div>
                                </div>
                        ...
                        <table className="min-w-full border text-sm text-left">
                            <thead className="bg-blue-100 text-black">
                                <tr>
                                    ...
                                    <th className="px-4 py-2 border">fromAccount</th>
                                ...
                                {currentPayments.length > 0 ? (
                                    currentPayments.map((payment, idx) => (
                                        <tr key={payment.id} className="bg-white border-t">
                                            ...
                                            <td className="px-4 py-2 border">{payment?.journalNo || '-'}</td>
                                            <td className="px-4 py-2 border">{payment?.fromAccount?._id || '-'}</td>
                                            <td className="px-4 py-2 border">{payment?.amount || '-'}</td>
                                            <td className="px-4 py-2 border">{payment?.toAccount?._id || '-'}</td>
                                            <td className="px-4 py-2 border">{payment?.toAmount || '0'}</td>
                                            <td className="px-4 py-2 border">{payment.narration || '0'}</td>
