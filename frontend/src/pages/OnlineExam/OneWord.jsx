...
function OneWord() {
    ...
    const [oneWordQue, setOneWordQue] = useState({
        type: "oneWord",
        questions: []
    });
    ...
    const validateForm = () => {
        ...
        if (!formData.answer.trim()) {
            alert('Please enter a one-word answer.');
            return false;
        }
        if (formData.answer.trim().split(/\s+/).length > 1) {
            alert('Please enter a single-word answer.');
            return false;
        }
        return true;
    };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        let updatedQuestions = [...oneWordQue.questions];
        ...
        // Log updated state
        console.log('Submitted One-Word Questions:', { ...oneWordQue, questions: updatedQuestions });
        alert('One-Word questions submitted! (Placeholder)');
        // Reset state
        setOneWordQue({ type: "oneWord", questions: [] });
        setFormData({
            question: '',
            marks: '',
            answer: ''
    ...
    const handleBack = () => {
        if (formData.question || formData.marks || formData.answer || oneWordQue.questions.length > 0) {
